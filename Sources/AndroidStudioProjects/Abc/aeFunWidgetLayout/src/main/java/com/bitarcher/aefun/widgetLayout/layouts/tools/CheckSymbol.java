...
public class CheckSymbol extends Mesh implements IEnableSetterEntity {
    ...
    public CheckSymbol(CheckButtonClickableEntity checkButtonClickableEntity, float x, float y, float width, float height) {
        super(x, y, width, height, CheckSymbol.getCoordinates(width, height), 6, DrawMode.TRIANGLE_STRIP, checkButtonClickableEntity.checkButtonLayout.getWidget().getTheme().getThemeManager().getResourceManager().getEngine().getVertexBufferObjectManager()); // dummy init
    ...
    {
        Color color;
        ITheme theme = this.checkButtonClickableEntity.checkButtonLayout.getWidget().getTheme();

        if(this.checkButtonClickableEntity.checkButtonLayout.getWidget().isEnabled())
        {
