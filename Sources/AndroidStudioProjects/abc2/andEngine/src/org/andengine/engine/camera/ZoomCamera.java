...
public class ZoomCamera extends BoundCamera {
	...
	protected void applySceneToCameraSceneOffset(final TouchEvent pSceneTouchEvent) {
		...
		if(zoomFactor != 1) {
			VERTICES_TMP[Constants.VERTEX_INDEX_X] = pSceneTouchEvent.getX();
			VERTICES_TMP[Constants.VERTEX_INDEX_Y] = pSceneTouchEvent.getY();

			MathUtils.scaleAroundCenter(VERTICES_TMP, zoomFactor, zoomFactor, this.getCenterX(), this.getCenterY()); // TODO Use a Transformation object instead!?!

			pSceneTouchEvent.set(VERTICES_TMP[Constants.VERTEX_INDEX_X], VERTICES_TMP[Constants.VERTEX_INDEX_Y]);
		}
	...
	protected void unapplySceneToCameraSceneOffset(final TouchEvent pCameraSceneTouchEvent) {
		...
		final float zoomFactor = this.mZoomFactor;
		if(zoomFactor != 1) {
			VERTICES_TMP[Constants.VERTEX_INDEX_X] = pCameraSceneTouchEvent.getX();
			VERTICES_TMP[Constants.VERTEX_INDEX_Y] = pCameraSceneTouchEvent.getY();

			MathUtils.revertScaleAroundCenter(VERTICES_TMP, zoomFactor, zoomFactor, this.getCenterX(), this.getCenterY()); // TODO Use a Transformation object instead!?!

			pCameraSceneTouchEvent.set(VERTICES_TMP[Constants.VERTEX_INDEX_X], VERTICES_TMP[Constants.VERTEX_INDEX_Y]);
		}
	}
